...
class Q_Session
{
	 ...
	 */
	static function regenerate_id($destroy_old_session = false, $duration = 0)
	{
		$previd = self::id(); // save old id
		session_regenerate_id(); // generate a new id
		$sid = self::id(); //  hang on to the new session id
		
		session_write_close(); // save close the session temporarily
		if ($destroy_old_session) {
			// destroy the old session file
			$sess_file = self::$session_save_path . DS . "sess_$previd";
	...
	{
		return str_replace('-', '', Q_Utils::uuid());
	}
