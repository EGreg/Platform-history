...
//		}
	 ...
	 * @param {boolean} [$throw_if_not_writeable=false] Defaults to false.
	 * Whether to throw a Q_Exception_NotWriteablePath if no user is logged in.
	 * @param {boolean} [$mkdir_if_missing] Defaults to false.
	...
	static function canWriteToPath(
		$path, 
		$throw_if_not_writeable = false, 
		$mkdir_if_missing = false
	) {
		$result = Q::event(
			"Q/Utils/canWriteToPath",
			compact('path', 'throw_if_not_writeable', 'mkdir_if_missing'),
			'before'
		);
		if (isset($result)) {
			return $result;
			...
			foreach ($paths as $p) {
				...
				if (strncmp($path, $p, $len) === 0) {
					...
					if ($mkdir_if_missing and !file_exists($path)) {
						$mask = is_string($mkdir_if_missing)
							? umask($mkdir_if_missing)
							: umask();
