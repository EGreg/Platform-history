...
class Q_Html
{
	 ...
	 *   an prefixes to which autogenerated ids will be appended
	 * @param {string|integer} [$selectedKey=null] Basically the value of the selected option
	 * @param {string} [$includeBlank=false] If null, don't include a blank option. 
	 ...
	 * @param {array} [$checked=array()] Associative array indicating which checkboxes are checked. If a key from $list 
	 ...
	 * @param {array} [$value=null] The value to represent using the resulting element. If many options present, tries to select this value.
		...
		$options = null)
	{
		...
		switch ($type) {
			case 'hidden':
				return self::hidden($value, isset($attributes['name']) ? $attributes['name'] : null);
	 ...
	 $unconvert = array())
	{
		...
		$result = htmlentities($content, ENT_QUOTES, 'UTF-8');
