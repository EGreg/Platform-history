...
class Q_Uri
{
	...
	{
		if (!isset($base_url)) {
			return isset(self::$cache_base_url) ? self::$cache_base_url : array();
		}
		$prev_base_url = self::$cache_base_url;
		self::$cache_base_url = $base_url;
	...
	protected static function matchSegments($pattern, $segments)
	{
		...
		if (substr($pattern, -2) === '[]') {
			...
			if (!in_array($last_rs[0], self::$variable_prefixes)) {
				return false;
			}
		...
		for ($i = 0; $i < $count; ++ $i) {
			...
			for ($j = 0; $j < $rs_parts_count; ++$j) {
				if (!isset($rs_parts[$j][0]) or !in_array($rs_parts[$j][0], self::$variable_prefixes)) {
					// literal value
					if ($s_parts[$j] !== str_replace(self::$escaped_variable_prefixes, self::$variable_prefixes, $rs_parts[$j])) {
						return false;
					}
					continue;
				}
	 ...
	 $controller = true)
	{
		...
		if (substr($pattern, -2) === '[]') {
			...
			if (false === in_array($last_rs[0], self::$variable_prefixes)) {
				return false;
			}
		...
		foreach ($rsegments as $rs) {			
			...
			$segment_parts = array();
			for ($j = 0; $j < $rs_parts_count; ++$j) {
				if (!isset($rs_parts[$j][0]) or (false === in_array($rs_parts[$j][0], self::$variable_prefixes))) {
					// literal value
					$segment_parts[] = urlencode(
						str_replace(self::$escaped_variable_prefixes, self::$variable_prefixes, $rs_parts[$j])
					);
					continue;
				}
				// otherwise, $variable
	...
	static function cachedUrl($url) {
		...
		and self::$cache_base_url) {
			return self::$cache_base_url . $url_relative_to_base;
		}
		return $url;
	}
	...
	/**
	 * @property $variable_prefixes
	 * @public
	 * @type array
	 */
	public static $variable_prefixes = array('$', ':');
	/**
	 * @property $variable_prefixes
	 * @public
	 * @type array
	 */
	public static $escaped_variable_prefixes = array('\$', '\:');
	/**
	 * @property $cache_base_url
	 * @public
	 * @type string
	 */
	protected static $cache_base_url = null;
	/**
	 * @property $urls
	 * @public
	 * @type string
