...
class Handlebars_Arguments
{
    ...
    {
        $bad_chars = preg_quote(Handlebars_Context::NOT_VALID_NAME_CHARS, '#');
        $bad_seg_chars = preg_quote(Handlebars_Context::NOT_VALID_SEGMENT_NAME_CHARS, '#');
     ...
     *
     * The method unescapes value and wrap it into Handlebars_String class if
     * needed.
     *
    ...
    protected function prepareArgumentValue($value)
    {
        ...
        if (preg_match("/^-?\d+$/", $value)) {
            // Wrap the value into the Handlebars_String class to tell the Handlebars_Context that
