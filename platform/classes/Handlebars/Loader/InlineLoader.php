 ...
 *
 * Handlebars Inline Handlebars_Template string Handlebars_Loader implementation.
 *
 * With the InlineLoader, templates can be defined at the end of any PHP source
 * file:
 *
 *     $loader  = new Handlebars_Loader_InlineLoader(__FILE__, __COMPILER_HALT_OFFSET__);
 ...
 *
 * Handlebars_Templates are deliniated by lines containing only `@@ name`.
 *
 * @category  Xamin
 * @package   Handlebars
 ...
 * @link      http://xamin.ir *
 */
class Handlebars_Loader_InlineLoader implements Handlebars_Loader
{
    protected $fileName;
    protected $offset;
    protected $templates;
    ...
    {
        if (!is_file($fileName)) {
            throw new InvalidArgumentException('Handlebars_Loader_InlineLoader expects a valid filename.');
        }

        if (!is_int($offset) || $offset < 0) {
            throw new InvalidArgumentException('Handlebars_Loader_InlineLoader expects a valid file offset.');
        }
