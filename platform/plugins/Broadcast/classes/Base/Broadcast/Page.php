 ...
 * Instead, change the Broadcast_Page.php file.
 *
 * @module Broadcast
 */
/**
 * Base class representing 'Page' rows in the 'Broadcast' database
 * @class Base_Broadcast_Page
 * @extends Db_Row
 */
abstract class Base_Broadcast_Page extends Db_Row
{
	/**
	...
	{
		if (Q_Config::get('Db', 'connections', 'Broadcast', 'indexes', 'Page', false)) {
			return new Db_Expression(($with_db_name ? '{$dbname}.' : '').'{$prefix}'.'page');
		} else {
			$conn = Db::getConnection('Broadcast');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'page';
  			if (!$with_db_name)
  				return $table_name;
	 ...
	 */
	function beforeSet_page_id($value)
	{
		if ($value instanceof Db_Expression) return array('page_id', $value);
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".page_id");
		if (strlen($value) > 255)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".page_id");
		return array('page_id', $value);			
	}
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_heading($value)
	{
		if ($value instanceof Db_Expression) return array('heading', $value);
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".heading");
		if (strlen($value) > 255)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".heading");
		return array('heading', $value);			
	}
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('page_id','publisherId') as $name) {
				if (!isset($value[$name])) {
	...
	{
		$field_names = array('page_id', 'publisherId', 'heading');
		$result = $field_names;
