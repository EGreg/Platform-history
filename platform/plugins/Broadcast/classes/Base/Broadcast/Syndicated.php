 ...
 * Instead, change the Broadcast_Syndicated.php file.
 *
 * @module Broadcast
 */
/**
 * Base class representing 'Syndicated' rows in the 'Broadcast' database
 * @class Base_Broadcast_Syndicated
 * @extends Db_Row
 */
abstract class Base_Broadcast_Syndicated extends Db_Row
{
	/**
	...
	{
		if (Q_Config::get('Db', 'connections', 'Broadcast', 'indexes', 'Syndicated', false)) {
			return new Db_Expression(($with_db_name ? '{$dbname}.' : '').'{$prefix}'.'syndicated');
		} else {
			$conn = Db::getConnection('Broadcast');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'syndicated';
  			if (!$with_db_name)
  				return $table_name;
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('userId','publisherId','ordinal','platform','streamName') as $name) {
				if (!isset($value[$name])) {
	...
	{
		$field_names = array('userId', 'publisherId', 'ordinal', 'platform', 'insertedTime', 'streamName');
		$result = $field_names;
