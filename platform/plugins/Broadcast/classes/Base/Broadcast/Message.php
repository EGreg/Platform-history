 ...
 * Instead, change the Broadcast_Message.php file.
 *
 * @module Broadcast
 */
/**
 * Base class representing 'Message' rows in the 'Broadcast' database
 * @class Base_Broadcast_Message
 * @extends Db_Row
 */
abstract class Base_Broadcast_Message extends Db_Row
{
	/**
	...
	{
		if (Q_Config::get('Db', 'connections', 'Broadcast', 'indexes', 'Message', false)) {
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('publisherId','streamName','ordinal','instructions') as $name) {
				if (!isset($value[$name])) {
	...
	{
		$field_names = array('publisherId', 'streamName', 'ordinal', 'instructions');
		$result = $field_names;
