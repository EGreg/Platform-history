 ...
 *	 "type": Required. Type of scroller to which scroll indicators will be bound.
 ...
 *	 "scroller": Required. Object of the scroller. For 'iScroll' type this is exact iScroll instance,
 *							 for 'scroller' this is jQuery object on which $.fn.scroller was previously called.
 *	 "orientation": Optional. Affects indicators positioning and should depend on scroll direction of container element.
 *									Can be value of 'v' or 'h'. Defaults to 'v'. On 'v' indicators are place at the top and bottom and on 'h'
 *									they are placed at the left and right.
 *	 "startClass": Optional. css class for indicator on the top or left (depending on scroll orientation).
 *								 May be used to change shadow image (background) and other styles.
 *	 "endClass": Optional. css class for indicator on the bottom or right (depending on scroll orientation).
 *							 May be used to change shadow image (background) and other styles.
 *	 "fadeTime": Optional. Time to fadeIn / fadeOut for indicators. Defaults to 200ms.
