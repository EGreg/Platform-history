 ...
 *	 "x": Optional. Integer which sets initial horizontal scroll position. Defaults to 0.
 *	 "y": Optional. Integer which sets initial vertical scroll position. Defaults to 0.
 *	 "inertia": Defaults to true. Boolean which indicates whether to apply 'intertial scrolling', meaning that after
 ...
 *	 "scrollbar": Optional. String value which can be either 'inner' or 'outer' determining where to actually place
 *								scrollbar element in the DOM tree. If the value is 'inner' then scrollbar is appended to the element to which
 *								plugin is applied. If the value is 'outer' then scrollbar is appended to document.body.
 *								The default value is 'inner'.
 *	 "hideScrollbar": Defaults to true. Boolean which indicates whether to hide scrollbar when no scrolling occurs.
 *	 "fadeScrollbar": Defaults to true. Boolean which indicates whether to fade scrollbars instead of just hiding them.
 *										Depends on 'hideScrollbar' options and works only it it's true.
 *	 "indicators": Defaults to true. Boolean which indicates whether to show scroll indicators
 *								 (apply Q/scrollIndicators plugin).
 *	 "onRefresh": Optional. Q.Event callback which is called when touchscroll('refresh') is called.
 *	 "onScrollStart": Optional. Q.Event callback which is called when scrolling is just started.
 *	 "onScrollMove": Optional. Q.Event callback which is called when scrolling is in progress
 *									 (while finger is moving or when inertial scrolling is applied).
 *	 "onScrollEnd": Optional. Q.Event callback which is called when scrolling is ended.
 *	 "onTouchEnd": Optional. Q.Event callback which is called when finger is released but scrolling still may continue
 *								 (due to inertial scrolling).
 *	 
