 ...
 * Instead, change the Awards_Earned.php file.
 *
 * @module Awards
 */
/**
 * Base class representing 'Earned' rows in the 'Awards' database
 * @class Base_Awards_Earned
 * @extends Db_Row
 *
 ...
 */
abstract class Base_Awards_Earned extends Db_Row
{
	/**
	 * @property $userId
	 * @type string
	...
	{
		if (Q_Config::get('Db', 'connections', 'Awards', 'indexes', 'Earned', false)) {
	...
	{
		if ($value instanceof Db_Expression) return array('badge_name', $value);
		if (!is_string($value) and !is_numeric($value))
	...
	{
		if ($value instanceof Db_Expression) return array('associated_id', $value);
		if (!is_string($value) and !is_numeric($value))
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('userId','app','badge_name','associated_id') as $name) {
				if (!isset($value[$name])) {
