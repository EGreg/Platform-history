 ...
 * Instead, change the Streams_RelatedTo.php file.
 *
 * @module Streams
 */
/**
...
abstract class Base_Streams_RelatedTo extends Db_Row
{
	...
	{
		if (Q_Config::get('Db', 'connections', 'Streams', 'indexes', 'RelatedTo', false)) {
			return new Db_Expression(($with_db_name ? '{$dbname}.' : '').'{$prefix}'.'related_to');
		} else {
			$conn = Db::getConnection('Streams');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'related_to';
  			if (!$with_db_name)
  				return $table_name;
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('toPublisherId','toStreamName','fromPublisherId','fromStreamName') as $name) {
				if (!isset($value[$name])) {
