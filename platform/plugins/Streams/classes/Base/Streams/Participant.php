...
abstract class Base_Streams_Participant extends Db_Row
{
	...
	{
		if (!isset($value)) return array('streamType', $value);
		if ($value instanceof Db_Expression) return array('streamType', $value);
		if (!is_string($value) and !is_numeric($value))
	...
	{
		if ($value instanceof Db_Expression) return array('subscribed', $value);
	...
	{
		if ($value instanceof Db_Expression) return array('posted', $value);
		if (!in_array($value, array('yes','no')))
