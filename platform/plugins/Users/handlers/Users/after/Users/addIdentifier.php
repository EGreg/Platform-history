...
function Users_after_Users_addIdentifier($params)
{
	...
	if (isset($email)) {
		$resend_button = "<button id='notices_set_email' class='Q_button' onclick='window.Q.plugins.Users.setIdentifier();'>Need it re-sent?</button>";
		Q_Response::setNotice('Users/email', "Please check your email for an activation link. $resend_button");
	} else if (isset($mobile)) {
		$resend_button = "<button id='notices_set_mobile' class='Q_button' onclick='window.Q.plugins.Users.setIdentifier();'>Need it re-sent?</button>";
